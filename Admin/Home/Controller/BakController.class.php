    ...
    public function index() {
        ...
        if (!empty($_GET['Action'])) {
            ...
            if ($_GET['Action'] == 'backup') {
                ...
                //$this->success( '数据库备份成功！', U("Bak/index"),1);
            } elseif ($_GET['Action'] == 'RL') {
                $mr->recover($_GET['File']);
                echo "<script>document.location.href='" . U("Bak/index") . "'</script>";
                //$this->success( '数据库还原成功！', U("Bak/index"),1);
            } elseif ($_GET['Action'] == 'Del') {
                if (@unlink($DataDir . $_GET['File'])) {
                    //$this->success('删除成功！',U("Bak/index"),1);
                    echo "<script>document.location.href='" . U("Bak/index") . "'</script>";
                } else {
                    $this->error('删除失败！');
                }
        ...
        $lists = $this->MyScandir(C('BACKUP_PATH'));
